...
function Modules({ detail, assessmentTitle }) {
  return <aside className="quiz-modules" aria-label="Daftar modul"><h2>Daftar Modul</h2><section><h3>Introduction to HR <span aria-hidden="true">⌃</span></h3>{detail.modules.flatMap((module) => module.lessons).map((lesson) => <div className="quiz-module-item" key={lesson.title}><span className="quiz-check" aria-hidden="true">✓</span><span>{lesson.title}<small>{lesson.minutes} Menit</small></span></div>)}<div className="quiz-module-item is-selected"><span aria-hidden="true">▣</span><span>{assessmentTitle}: Introduction to HR<small>10 Pertanyaan</small></span></div></section><button className="quiz-review" type="button">☆ Beri Review & Rating</button></aside>;
}
