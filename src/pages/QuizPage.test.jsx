...
test('renders the first quiz question and question list', () => {
  open();
...
test('moves through questions and finishes on the last question', () => {
  open();
...
test('selects an answer and marks the question as answered', () => {
  open();
...
test('confirms finishing the assessment with a dynamic modal', () => {
  open();
  fireEvent.click(screen.getByRole('button', { name: '10' }));
...
test('shows a completed result after confirming submit', () => {
  open();
...
test('shows try again when the score is below the passing grade', () => {
  open();
  fireEvent.click(screen.getByRole('button', { name: '10' }));
  fireEvent.click(screen.getByRole('button', { name: /^Selesaikan/  }));
  fireEvent.click(screen.getByRole('button', { name: 'Selesai' }));
...
test('passes when at least six answers are correct', () => {
  open();
...
test.each([['pretest', 'Pre-Test'], ['quiz', 'Quiz'], ['exam', 'Ujian Akhir']])('retry %s returns to its submodule and starts a fresh attempt', (assessment, title) => {
  open(assessment);
