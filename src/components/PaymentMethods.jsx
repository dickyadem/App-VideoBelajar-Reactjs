...
import { paymentLogos } from '../data/paymentMethods';
import { useCatalog } from '../context/CatalogContext';

export default function PaymentMethods({ selectedPayment, onChange, disabled = false }) {
  const { paymentGroups } = useCatalog();
  const [openGroup, setOpenGroup] = useState(() => paymentGroups.find((group) => group.id === 'bank')?.id || paymentGroups[0]?.id);

  if (!paymentGroups.length) return <p role="status">Metode pembayaran belum tersedia.</p>;
      ...
      return <div className="payment-group" key={group.id}>
        ...
        <div id={`payment-options-${group.id}`} hidden={!isOpen}>
          {group.options.map((option) => <label className="payment-option" key={option.id}>
            <input type="radio" name="payment-method" value={option.id} checked={selectedPayment === option.id} disabled={disabled}
            ...
            {option.brands ? <span className="payment-card-brands" aria-hidden="true">
              ...
              {(option.logo || paymentLogos[option.id]) && <img className="payment-logo-image" src={option.logo || paymentLogos[option.id]} alt="" />}
