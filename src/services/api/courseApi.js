import { addDoc, collection, deleteDoc, doc, getDocsFromServer, updateDoc } from 'firebase/firestore';
...
export async function createCourse(course) {
  const reference = await addDoc(collection(db, 'courses'), course);
  return { ...course, id: reference.id };
}

export async function updateCourse(id, changes) {
  await updateDoc(doc(db, 'courses', id), changes);
  return { id, ...changes };
}
